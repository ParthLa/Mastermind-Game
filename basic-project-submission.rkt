...
(define (update-online-world w x y)
  (cond [(inside? back-button-location x y) (set! w "play-world1")]
        [(inside? start-button-location x y) (set! ip-address1 LOCALHOST)
                                             (set! code-no 1)
                                             (set! w "run-universe")]
        [(inside? join-button-location x y) (set! w "online-join-world-practice")])
  w)
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;online-start-window;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;(define (online-start-window)
;  (set! code-no 1)
;  (set! ip-address1 LOCALHOST)
  
                   ;(send ip-address-field set-value LOCALHOST)
                   ;(send fr show #f)
                   ;(cond [(not ran-universe) (begin (set! ran-universe #t) (run-universe))])
  ;(create-world "online-start-game-world")) 

;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;online-join-window;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

(define (online-join-window stat)
  (set! code-no 1)
  (send fr show #t)
  stat)
;  (set-from-where-to-guess-list2!)
;  (set! ip-address2 (send ip-address-field get-value))
;                   (send fr show #f)
;                   (create-world "online-join-game-world"))
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;online-start-game-window;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

(define (update-choose-code-world2 w x y)
  (cond [(equal? (length code-list) (car (choose-from)))
                  (set! w "online-start-game-world2") (set! colour-ball-vector (list->vector colour-ball-list)) w]
  [(inside? back-button-location x y) (set! w "quit-world") w]
        [else (update-choose-code-world-helper w x y 0)]))

(define (online-start-game-window2)
  (set! x (/ (* width 0.56) (car (choose-from))))                    ;; x is the x-length of square
  (set! y (/ (* 7 height) (* 8 (no-of-trials))))  
  (set! sq-p (rectangle x y 'outline 'black))
  (set! sq-s (rectangle (/ x 2) (/ y 2) 'outline 'black))
...
(define (update-game-world2 w x y)
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;online-join-game-window;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

(define (update-levels-game-world2 w x y)
  (cond [(or (inside? done-button-location x y) (equal? (length single-guess-list) (car (choose-from))))
                  (begin
                  (set! w "levels-game-world2")
                  (input2 single-guess-list)
                  ...
                  ;(if (or (> levels-guess-no (no-of-trials)) (equal? (car (choose-from)) black-cnt)) (begin (set! w "levels-game-finish") w)
                  ;   w))]
                  (make-package w single-guess-list-no))]
  ;(cond [(inside? back-button-location x y) (set! w "set-world") w]
        [else (update-levels-game-world-helper w x y 0)]))
...
(define (f x)
  (overlay/align "middle" "bottom" bottom-rectangle x))

(define (g2 x cnt)
  (begin (set! white-cnt (car x))
         (set! black-cnt (cdr x))
         (place-black-white cnt)))
;  (place-images (take colour-ball-list 6)
;                (build-list (cdr (choose-from)) (lambda (x) (make-posn (* (+ 1 x) (/ width 7)) button-center-y)))
;                x))
                

(define (g1 x cnt)
  (begin (set! levels-guess-no cnt)
         (input2 (convert-to-ball-list x)))
  )

(define (add-world univ wrld)
  (let ((univ* (append univ (list wrld))))
    (make-bundle univ*
                 (cond ((= 1 (length univ*))
                        (list (make-mail (first univ*) "waiting for other player")))
                       ((= 2 (length univ*))
                        (list (make-mail (second univ*) "waiting for other player's move")
                              (make-mail (first univ*) "start and play the move")))
                       ((= 3 (length univ*))
                        (list (make-mail (first (reverse univ*)) "sorry, already 2 players playing"))))
                     (if (= 3 (length univ*)) (list (first (reverse univ*)))
                                                  null))))

(define (switch univ wrld m)  
  (local ((define univ* (reverse univ)))
    (make-bundle univ*
                 (cond [(pair? m) (cond [(= cnt (cdr m)) (displayln "the codebreaker won") (set! wrld "start-world") ])]
                       [(= 11 cnt) (displayln "the codemaker won") (set! wrld "start-world")]
                       [(= 1 (length univ*)) (list (make-mail (car univ*) "waiting for other player"))]
                       [else (cond [(equal? m (cons white-cnt black-cnt))   (list (make-mail (car univ*) (g2 m cnt)))]
                                                                    
                                                                                                  
                                   [else (set! cnt (+ cnt 1)) (list (make-mail (car univ*) (g1 m cnt)))
                                                                    ])])
                                                                              ; m is list of colour balls or pair of black-white pegs...
                '())) (set! cnt (+ cnt 1)))

(define (disconnect-expr univ wrld)
  ;(set! no-of-rounds-played (+ no-of-rounds-played 1))
  (local ((define univ* (remove wrld univ)))
    (make-bundle univ*
                 (list (make-mail (first univ*) "other player is offline now"))
                 (list wrld))))
(define ran-universe #f)
(define (start-server)
  (universe '() (on-new add-world) (on-msg switch) (on-disconnect disconnect-expr)))

(define (run-universe) (start-server)) ; how to use run-universe to create universe 

;; to be added in start(main program)...
(define fr
  (new frame%
       [label "Ip-address"]
       [width 150]
...
(define ip-address1 "")
(define ip-address2 "")
(define ONLINE-JOIN-WORLD? #f)
(define JOIN-WORLD? #f)
;(define msg (new message% [label "Note: If you want to be host, please press the run button first"]
;                          [parent frame]))
(define ip-address-field (new text-field%
                              [label "IP-ADDRESS"]
                              [parent fr]
                              [callback (λ (t e) (cond [(equal? (send e get-event-type) 'text-field-enter)
                                                        (set! ip-address2 (send ip-address-field get-value))
                                                        (set! ONLINE-JOIN-WORLD? #t)
                                                        (send fr show #f)]))]))
;(define start
;  (new button%
;       [label "start"]
;       [callback (λ (b e) (set! ip-address1 LOCALHOST)
;                   (send ip-address-field set-value LOCALHOST)
;                   ;(send fr show #f)
;                   (run-universe))]
;       [vert-margin 5])) ; this is the host that will open the server and add the other worlds onto it.

;(define join
;  (new button%
;       [label "join"]
;       [callback (λ (b e) (set! ip-address2 (send ip-address-field get-value))
;                   (send fr show #f)
;                   (create-world player-name))]))

;(send fr show #t)
(define MADE-OFFLINE? #f)
(define (create-world player-name)
  (set! MADE-OFFLINE? #f)
  (big-bang player-name
  (to-draw next-window)
  ...
  (on-tick (lambda (stat) (cond [call-game-world? (set! call-game-world? #f)
                                             ...
                                             "game-world2"]
                                [else stat])))
  (stop-when (λ (stat) (begin0 (equal? stat "quit-world")
                               (cond [(and (equal? stat "quit-world")
                                           (not MADE-OFFLINE?))
                                      (set! MADE-OFFLINE? #t)
                                      (main-world-make)]))))
  (close-on-stop #t)
  (register ip-address1)));connects this world to a universe server at the specified ip-expr address and set up capabilities for sending and receiving messages
;(define (world-make) (add-world-in-univ))
...
(define (next-window w)
  (cond [(equal? w "start-world") (start-window)]
        ...
        ;[(equal? w "online-start-world") (online-start-window)]
        [(equal? w "online-join-world-practice") (online-window)]
        [(equal? w "online-join-world") (online-join-window)]
        [(equal? w "online-start-game-world") (choose-code-window)]
        [(equal? w "online-start-game-world2") ((online-start-game-window2))]
        [(equal? w "online-join-game-world") (levels-game-window)]
        ))
...
(define (highlight w x y)
  (cond [(equal? w "start-world") (highlight-start-world w x y)]
        ...
        [(equal? w "online-start-game-world") (highlight-choose-code-world w x y)]
        [(equal? w "quit-world") (highlight-choose-code-world w x y)];An image to be applied
        [(equal? w "online-join-world-practice") (highlight-online-world w x y)]
        [(equal? w "run-universe") (highlight-online-world w x y)]
        [else w]))
...
(define (update w x y)
  (cond [(equal? w "start-world") (update-start-world w x y)]
        ...
        [(equal? w "online-join-world-practice") (send fr show #t)
                                                 (update-online-world w x y)]
        [(equal? w "run-universe") (set! ip-address1 LOCALHOST)
                                   (set! code-no 1)
                                   (update-online-world w x y)]
        ...
        [(equal? w "online-start-game-world") (update-choose-code-world2 w x y)]
        [(equal? w "quit-world") (update-choose-code-world2 w x y)]
        [(equal? w "online-start-game-world2") (update-game-world2 w x y)]
        [(equal? w "online-join-game-world") (update-levels-game-world2 w x y)]
...
(define ONLINE-START-WORLD? #f)

(define (main-world-make)
  (set! ONLINE-JOIN-WORLD? #f)
  (set! ONLINE-START-WORLD? #f)
  (big-bang "start-world"
  (to-draw next-window)
  (on-mouse choose-next)
  (on-tick (lambda (stat) (cond [OPEN-WORLD? (set! OPEN-WORLD? #f)
                                             (set! ONLINE-START-WORLD? #t)
                                             stat]
                                [call-game-world? (set! call-game-world? #f)
                                             (set! sc (game-window2))
                                             "game-world2"]
                                [call-game-world2? (set! call-game-world2? #f)
                                                   ...
                                                   "genetic-game-world2"]
                                [else stat])))
  (stop-when (λ (stat) (begin0 (or (equal? stat "quit-world")
                                   (equal? stat "run-universe")
                                   ONLINE-START-WORLD?
                                   ONLINE-JOIN-WORLD?)
                               (cond [(and ONLINE-JOIN-WORLD? (not JOIN-WORLD?))
                                      (set! JOIN-WORLD? #t)
                                      (displayln "Hello")
                                      (create-world "online-join-game-world")]
                                     [(and ONLINE-START-WORLD? (not JOIN-WORLD?))
                                      (set! JOIN-WORLD? #t)
                                      (displayln "Hello1")
                                      (create-world "online-start-game-world")]
                                     [(and (equal? stat "run-universe")
                                           (not UNIVERSE-RUNNING?))
